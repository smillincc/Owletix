  ...
  if (success) return (
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", sans-serif' }}>
      <div style={{ textAlign: 'center', maxWidth: '380px', padding: '2rem' }}>
        ...
        <Link href="/auth/login" style={{ backgroundColor: '#1d1d1f', color: '#fff', textDecoration: 'none', padding: '0.875rem 2rem', borderRadius: '980px', fontWeight: '600', fontSize: '0.95rem', display: 'inline-block' }}>Sign in →</Link>
      </div>
    </div>
  );
      ...
      <div style={{ width: '100%', maxWidth: '460px' }}>
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '1.5rem', padding: '2.25rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.06)' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem', marginBottom: '1.75rem' }}>
            {[{ value: 'CUSTOMER', icon: '👁️', label: 'Book aerial views', sub: 'I want to see from above' }, { value: 'PILOT', icon: '🚁', label: 'Fly & earn', sub: 'I am a drone pilot' }].map(r => (
              <button key={r.value} onClick={() => update('role', r.value)} type="button" style={{ backgroundColor: form.role === r.value ? '#f0f0ff' : '#f5f5f7', border: `2px solid ${form.role === r.value ? '#6366f1' : 'transparent'}`, borderRadius: '1rem', padding: '1rem', cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s', fontFamily: 'inherit' }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.875rem' }}>
            ...
            <div>
              <label style={{ display: 'block', fontSize: '0.8rem', fontWeight: '600', color: '#1d1d1f', marginBottom: '0.35rem' }}>Email address</label>
            ...
            <button type="submit" disabled={loading} style={{ backgroundColor: '#6366f1', color: '#fff', border: 'none', borderRadius: '980px', padding: '0.9rem', fontSize: '0.95rem', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1, marginTop: '0.25rem', fontFamily: 'inherit' }}>
              {loading ? 'Creating account...' : `Create ${form.role === 'PILOT' ? 'pilot ' : ''}account →`}
            </button>
            <p style={{ fontSize: '0.72rem', color: '#6e6e73', textAlign: 'center', lineHeight: '1.5' }}>By signing up you agree to our Terms of Service and Privacy Policy</p>
          </form>
        </div>
