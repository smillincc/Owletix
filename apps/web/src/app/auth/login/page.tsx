...
export default function LoginPage() {
  ...
  const inp = { width: '100%', backgroundColor: '#fff', border: '1px solid rgba(0,0,0,0.12)', borderRadius: '0.75rem', padding: '0.875rem 1rem', color: '#1d1d1f', fontSize: '0.95rem', outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: '#f5f5f7', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", sans-serif' }}>
      <div style={{ width: '100%', maxWidth: '420px' }}>
        ...
        <div style={{ backgroundColor: '#fff', borderRadius: '1.5rem', padding: '2.25rem', boxShadow: '0 4px 24px rgba(0,0,0,0.06)', border: '1px solid rgba(0,0,0,0.06)' }}>
          {error && (
            <div style={{ backgroundColor: '#fff2f2', border: '1px solid #fecaca', borderRadius: '0.75rem', padding: '0.875rem 1rem', marginBottom: '1.5rem', color: '#dc2626', fontSize: '0.875rem' }}>{error}</div>
          )}
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            ...
            <button type="submit" disabled={loading} style={{ backgroundColor: '#1d1d1f', color: '#fff', border: 'none', borderRadius: '980px', padding: '0.9rem', fontSize: '0.95rem', fontWeight: '600', cursor: loading ? 'not-allowed' : 'pointer', opacity: loading ? 0.6 : 1, marginTop: '0.5rem', fontFamily: 'inherit' }}>
